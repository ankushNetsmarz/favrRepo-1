...
@interface SocialSyncVC ()<GPPSignInDelegate,GPPShareDelegate>{
...
static NSString * const kClientId = @"833933554327-hqpdf3nvhlm4qlq5qq3fifckhnko0ibl.apps.googleusercontent.com";
    ...
    if (sender.on) {
        switch (sender.tag) {
            case kFBSwitch:
            {
                ...
                if (fbProfileDetailsDict.count==0) {
                    return;
                }
...
//                            int tweets = [[(NSDictionary *)TWData objectForKey:@"statuses_count"] integerValue];
            ...
            {
                NSLog(@"Please configure your Twitter account in setting");
                
                UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"" message:@"Please configure your Twitter account in setting." delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
                [alert show];
            }
        } else {
